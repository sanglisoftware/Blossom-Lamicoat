...
const handleFilterChange = (value: string) => {
  ...
  searchTimeout.current = setTimeout(() => {
    ...
    if (tabulator.current) {
      tabulator.current.setData(`${BASE_URL}/api/grade`, {
        params: {
          page: 1,
  ...
  const handleDeleteGrade = async () => {
    ...
    try {
      await axios.delete(`${BASE_URL}/api/grade/${deleteGradeId}`, {
        headers: { Authorization: token ? `Bearer ${token}` : "" },
      });
  ...
  return (
    <>
      <div className="flex flex-col sm:flex-row items-center mt-8 mb-4">
        <h2 className="mr-auto text-lg font-medium">Grade Table</h2>
        ...
        <Dialog.Panel>
          <div className="p-5 text-center">
            ...
            <div className="mt-2 text-slate-500">
              Do you really want to <span className="text-danger">delete</span> this Grade?
            </div>
          </div>
