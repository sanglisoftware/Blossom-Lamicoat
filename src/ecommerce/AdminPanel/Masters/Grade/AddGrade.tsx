...
interface CreateNewGradeModalProps {
  open: boolean;
  onClose: () => void;
  onSuccess: () => void; 
}
...
const AddGrade: React.FC<CreateNewGradeModalProps> = ({
  open,
  onClose,
  onSuccess
}) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${BASE_URL}/api/grade`, payload, {
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        setSuccessModalConfig({
          ...
          subtitle: "The new grade has been added to the system.",
          icon: "CheckCircle",
          buttonText: "OK",
