...
type MixtureApproveRow = {
  id: number;
  Mixture: string;
  MixtureRollNo: string;
  FinalProduct: string;
  Chem1Qty: number;
  Chem2Qty: number;
  Chem3Qty: number;
...
function Main() {
  ...
  const [tableData, setTableData] = useState<MixtureApproveRow[]>([]);

  useEffect(() => {
    const fetchMixtureApproveReport = async () => {
      try {
          ...
          items.map(async (item, index) => {
            ...
            if (formulaMasterId > 0) {
              try {
                ...
                console.error("Error fetching mixture approve chemicals:", error);
              }
            }
            ...
            return {
              ...
              Mixture: `Mixture ${index + 1}`,
              MixtureRollNo: `MROLL-${String(index + 1).padStart(3, "0")}`,
              FinalProduct: String(
        ...
        console.error("Error fetching mixture approve report:", error);
      }
    };

    fetchMixtureApproveReport();
  }, [token]);
  ...
  useEffect(() => {
    ...
    tabulator.current = new Tabulator(tableRef.current, {
      ...
      columns: [
        ...
        { title: " Roll No", field: "MixtureRollNo" },
  ...
  const handleSearch = (value: string) => {
    ...
    tabulator.current?.setFilter((row: MixtureApproveRow) =>
      [
        row.Mixture,
        row.MixtureRollNo,
        row.FinalProduct,
  ...
  return (
    <>
      <div className="flex items-center justify-between mt-8 mb-4">
        <h2 className="text-lg font-medium">Mixture Approve Report</h2>
      </div>
