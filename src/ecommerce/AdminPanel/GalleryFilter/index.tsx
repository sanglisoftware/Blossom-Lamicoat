...
import EditFilterModal from "./EditGalleryFilter";
...
function Main() {
    ...
    const [addNewFilterModalPreview, setAddFilterModalPreview] = useState(false);
    ...
    const [editFilterModalPreview, setEditFilterModalPreview] = useState(false);
    ...
    const handleSubmit = async () => {
        ...
        if (!formData.filterName) errors.roleName = "Filter is required.";
        //setting errors
        setFormErrors(errors);
        if (Object.keys(errors).length > 0) return;

        try {
            const payload = { filterValue: formData.filterName };
            const response = await axios.post(
                `${BASE_URL}/api/GalleryFilters`,
                payload,
                {
                    headers: {
            ...
            if (response.status === 200 || response.status === 201) {
                ...
                setAddFilterModalPreview(false);
                //success modal
                setSuccessModalConfig({
                    title: "Filter Added Successfully",
                    subtitle: "The new Filter has been saved to the system.",
                    icon: "CheckCircle",
                    buttonText: "Ok",
                    onButtonClick: () => {
    ...
    const initTabulator = () => {
        if (tableRef.current) {
            tabulator.current = new Tabulator(tableRef.current, {
                ajaxURL: `${BASE_URL}/api/GalleryFilters/tabulator`,
                ajaxConfig: {
                    method: "GET",
                    headers: {
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        formatter(cell) {
                            const response: any = cell.getData();
                            return `<div>
                        <div class="font-medium whitespace-nowrap">${response.filterValue}</div> </div>`;
                        },
                    },
                    {
                        title: "ACTIONS",
                            ...
                            const actions = [
                                {
                                    ...
                                    onClick: () => {
                                        ...
                                        setEditingFilterId(rowData.id)
                                        setEditFilterModalPreview(true)
                                    },
                                },
                                // {
                                //     label: "Delete",
    ...
    return (
        <>
            ...
            <div className="flex flex-col items-center mt-8 intro-y sm:flex-row">
                <h2 className="mr-auto text-lg font-medium">Gallery Filters</h2>
                <div className="flex w-full mt-4 sm:w-auto sm:mt-0">
                    <Button
                        variant="primary"
                        className="mr-2 shadow-md"
                        onClick={() => {
                            setAddFilterModalPreview(true);
                        }}
                    >
                        Add New Filter
                    </Button>
                ...
                <Dialog.Panel>
                    ...
                    <Dialog.Description className="grid grid-cols-12 gap-4 gap-y-3">
                        <div className="col-span-12 sm:col-span-12">
                            <FormLabel htmlFor="size">Filter</FormLabel>
                            <FormInput
                                id="size"
                                type="text"
                                placeholder="Role"
                                ...
                                onChange={(e) => {
                                    ...
                                    setFormData({ ...formData, filterName: e.target.value })
                                    if (value.trim()) {
                                        setFormErrors((prev) => ({ ...prev, filterName: "" }));
                                    }
                                }}
                            />
                            {formErrors.filterName && <p className="text-red-500 text-sm">{formErrors.filterName}</p>}
                        </div>
                    </Dialog.Description>
                    <Dialog.Footer>
                        <Button
            ...
            {/* BEGIN: Edit Existing Filter Modal Content */}
            <EditFilterModal onClose={() => { setEditFilterModalPreview(false) }} open={editFilterModalPreview} filterID={editingFilterId} onSuccess={refreshData} />
            {/* BEGIN: Edit Existing Filter Modal Content */}
