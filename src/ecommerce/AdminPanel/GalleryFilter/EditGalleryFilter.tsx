...
const EditFilterModal: React.FC<EditFilterModalProps> = ({
    ...
    const handleSubmit = async () => {
        ...
        if (!formData.filterName) errors.filterName = "filter is required.";
        //setting errors
        setFormErrors(errors);
        if (Object.keys(errors).length > 0) return;

        try {
            const payload = { id: filterID, filterValue: formData.filterName };
            const response = await axios.put(
                `${BASE_URL}/api/GalleryFilters/${filterID}`,
                payload,
                {
                    headers: {
            ...
            if (response.status === 200 || response.status === 201) {
                ...
                setSuccessModalConfig({
                    ...
                    subtitle: "The Filter has been updated to the system.",
                    icon: "CheckCircle",
                    buttonText: "Ok",
                    onButtonClick: () => {
