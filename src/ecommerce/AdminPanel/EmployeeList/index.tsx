...
import CreateNewEmpoyeeModal from "./CreateNewEmployee";
import axios from "axios";
import EditEmpoyeeModal from "./EditEmployee";
    ...
    const initTabulator = () => {
        if (tableRef.current) {
            tabulator.current = new Tabulator(tableRef.current, {
                ajaxURL: `${BASE_URL}/api/employees/tabulator`,
                ajaxConfig: {
                    method: "GET",
                    headers: {
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        formatter(cell) {
                            ...
                            return `<div>
                        <div class="font-medium whitespace-nowrap">${fullname}</div>
                        <div class="text-slate-500 text-xs whitespace-nowrap">${response.roleValue}</div>
                        </div>`;
                        },
                    },
                    {
                        ...
                        formatter(cell) {
                            const response: any = cell.getData();
                            return `<div>
                        <div class="font-medium whitespace-nowrap">${response.roleValue}</div>
                        </div>`;
                        },
                    },
                    {
  ...
  formatter(cell) {
    ...
    `;
...

                    {
                        ...
                        formatter(cell) {
                            const response: any = cell.getData();
                            const isChecked = response.activeStatus === 0 ? "" : "checked";
                            return `<input 
                     ${isChecked}
                      onchange='
    ...
    return (
        <>
            ...
            <CreateNewEmpoyeeModal open={addNewEmployeeModalPreview} onClose={() => setaddNewEmployeeModalPreview(false)} onSuccess={refreshTableData} />
