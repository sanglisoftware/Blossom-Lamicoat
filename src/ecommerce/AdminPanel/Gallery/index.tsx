...
import AddNewGalleryModal from "./AddNewGalleryModal";
import EditGalleryModal from "./EditGallery";
//import EditGalleryModal from "./EditGallery";
...
function Main() {
    ...
    const [addNewGalleryModalPreview, setaddNewGalleryModalPreview] = useState(false);
    const [editGalleryModalPreview, setEditGalleryModalPreview] = useState(false);
        ...
        debounceTimer = setTimeout(async () => {
            try {
                const response = await fetch(
                    `${BASE_URL}/api/inventory/Gallery/${activeStatusRowId}/status`,
                    {
                        method: "PATCH",
                        headers: {
                ...
                if (response.ok) {
                    ...
                    setNotification({
                        message: `🔐 Gallery Image ${isActiveCategory === "0" ? "Hide " : "Show"} successfully`,
                        type: "success"
                    });
    ...
    const initTabulator = () => {
        if (tableRef.current) {
            tabulator.current = new Tabulator(tableRef.current, {
                ajaxURL: `${BASE_URL}/api/inventory/Gallery`,
                ajaxConfig: {
                    method: "GET",
                    headers: {
                ...
                columns: [
                            ...
                            input.addEventListener("input", (e) => {
                                ...
                                if (newValue !== "") {
                                    ...
                                    debounceTimer = setTimeout(async () => {
                                        ...
                                        try {
                                            const response = await fetch(
                                                `${BASE_URL}/api/inventory/Gallery/${rowData.id}/sequence`,
                                                {
                                                    method: "PATCH",
                                                    headers: {
                                                        "Content-Type": "application/json",
                    ...
                    },
                    {
                        ...
                        formatter(cell) {
                            ...
                            return `<div>
                  <div class="font-medium whitespace-nowrap">${response.filterName}</div>
                  <div class="text-slate-500 text-xs whitespace-nowrap">${response.filterName}</div>
                </div>`;
                        },
                    },
                    {
                            ...
                            const actions = [
                                {
                                    ...
                                    onClick: () => {
                                        const rowData = cell.getRow().getData();
                                        setEditingGalleryId(rowData.id);
                                        setEditGalleryModalPreview(true);
    ...
    return (
        <>
            ...
            <div className="flex flex-col items-center mt-8 intro-y sm:flex-row">
                <h2 className="mr-auto text-lg font-medium">Image Gallery</h2>
                <div className="flex w-full mt-4 sm:w-auto sm:mt-0">
                    <Button
                        variant="primary"
                        className="mr-2 shadow-md"
                        onClick={() => {
                            setaddNewGalleryModalPreview(true);
                        }}
                    >
                ...
                <Dialog.Panel>
                    <div className="p-5 text-center">
                        ...
                        <div className="mt-2 text-slate-500">
                            Do you really want to <strong>{statusLabel}</strong> this Gallery Image? <br />
                        </div>
                    </div>
            ...
            <AddNewGalleryModal open={addNewGalleryModalPreview} onClose={() => setaddNewGalleryModalPreview(false)} onSuccess={refreshTableData} />
            {/* END:  Add new Collection Modal Content */}

            {/* BEGIN: Add new Collection Modal Content */}
            <EditGalleryModal open={editGalleryModalPreview} onClose={() => setEditGalleryModalPreview(false)} onSuccess={refreshTableData}
