...
import AddSliderImageModal from "./AddSliderModal";
import EditSliderImageModal from "./EditSliderModal";
...
function Main() {
    ...
    const [addNewSliderModalPreview, setaddNewSliderImageModalPreview] = useState(false);
    const [editSliderImageModalPreview, setEditSliderImageModalPreview] = useState(false);
    ...
    const [editingSliderImageId, setEditingSliderImageId] = useState<number | null>(null);
        ...
        debounceTimer = setTimeout(async () => {
            try {
                const response = await fetch(
                    `${BASE_URL}/api/inventory/Slider/${activeStatusRowId}/status`,
                    {
                        method: "PATCH",
                        headers: {
                ...
                if (response.ok) {
                    ...
                    setNotification({
                        message: `🔐 Slider Image ${isActiveCategory === "0" ? "Hide " : "Show"} successfully`,
                        type: "success"
                    });
    ...
    const initTabulator = () => {
        if (tableRef.current) {
            tabulator.current = new Tabulator(tableRef.current, {
                ajaxURL: `${BASE_URL}/api/inventory/Slider`,
                ajaxConfig: {
                    method: "GET",
                    headers: {
                            ...
                            input.addEventListener("input", (e) => {
                                ...
                                if (newValue !== "") {
                                    ...
                                    debounceTimer = setTimeout(async () => {
                                        ...
                                        try {
                                            const response = await fetch(
                                                `${BASE_URL}/api/inventory/Slider/${rowData.id}/sequence`,
                                                {
                                                    method: "PATCH",
                                                    headers: {
                                                        "Content-Type": "application/json",
                            ...
                            const actions = [
                                {
                                    ...
                                    onClick: () => {
                                        const rowData = cell.getRow().getData();
                                        setEditingSliderImageId(rowData.id);
                                        setEditSliderImageModalPreview(true);
    ...
    return (
        <>
            ...
            <div className="flex flex-col items-center mt-8 intro-y sm:flex-row">
                <h2 className="mr-auto text-lg font-medium">Slider Images</h2>
                <div className="flex w-full mt-4 sm:w-auto sm:mt-0">
                    <Button
                        variant="primary"
                        className="mr-2 shadow-md"
                        onClick={() => {
                            setaddNewSliderImageModalPreview(true);
                        }}
                    >
                ...
                <div className="flex flex-col sm:flex-row sm:items-end xl:items-start">
                    <div className="xl:flex sm:mr-auto">
                        <div className="items-center mt-2 sm:flex sm:mr-4 xl:mt-0">
                            <label className="hidden flex-none w-12 mr-2 xl:w-auto xl:flex-initial">
                                Search
                            </label>
                            <FormInput
                                value={filterValue}
                                ...
                                type="text"
                                className="hidden mt-2 sm:w-40 2xl:w-full sm:mt-0"
                ...
                <Dialog.Panel>
                    <div className="p-5 text-center">
                        ...
                        <div className="mt-2 text-slate-500">
                            Do you really want to <strong>{statusLabel}</strong> this Slider Image? <br />
                        </div>
                    </div>
            ...
            <AddSliderImageModal open={addNewSliderModalPreview} onClose={() => setaddNewSliderImageModalPreview(false)} onSuccess={refreshTableData} />
            {/* END:  Add new Collection Modal Content */}

            {/* BEGIN: Add new Collection Modal Content */}
            <EditSliderImageModal open={editSliderImageModalPreview} onClose={() => setEditSliderImageModalPreview(false)} onSuccess={refreshTableData}
                sliderImageId={editingSliderImageId} />
            {/* END:  Add new Collection Modal Content */}
