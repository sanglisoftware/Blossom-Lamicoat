...
interface AddSliderImageProps {
    open: boolean;
    onClose: () => void;
    onSuccess?: () => void;
}

const AddSliderImageModal: React.FC<AddSliderImageProps> = ({ open, onClose, onSuccess }) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const response = await axios.post(`${BASE_URL}/api/inventory/Slider/upload-image?sequenceNo=${0}&isActive=${1}`, data, {
                headers: {
            ...
            if (response.status === 200 || response.status === 201) {
                ...
                setSuccessModalConfig({
                    title: "Slider Image Successfully",
                    subtitle: "The new Slider Image has been saved to the system.",
                    icon: "CheckCircle",
                    buttonText: "Ok",
                    onButtonClick: () => {
...
export default AddSliderImageModal;