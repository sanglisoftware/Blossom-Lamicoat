...
interface EditSliderImageModalProps {
    open: boolean;
    onClose: () => void;
    onSuccess?: () => void;
...
const EditSliderImageModal: React.FC<EditSliderImageModalProps> = ({
    open,
    onClose,
    onSuccess,
    sliderImageId
    ...
    useEffect(() => {
        const fetchSliderData = async () => {
            if (open && sliderImageId) {
                setIsLoading(true);
                try {
                    const response = await axios.get(
                        `${BASE_URL}/api/inventory/Slider/${sliderImageId}`,
                        {
                            headers: {
                    ...
                    setFormData({
                        sequenceNo: slider.sequenceNo,
                        isActive: slider.isActive
                    });

                    if (slider.imagePath) {
                        setExistingImage(`${BASE_URL}${slider.imagePath}`);
                    }
                } catch (error) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const response = await axios.put(
                `${BASE_URL}/api/inventory/Slider/update-with-image/${sliderImageId}?${params}`,
                imageFormData,
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
            ...
            if (response.status === 200) {
                ...
                setSuccessModalConfig({
                    ...
                    subtitle: "The SLider has been updated in the system.",
                    icon: "CheckCircle",
                    buttonText: "Ok",
                    onButtonClick: () => {
...
export default EditSliderImageModal;