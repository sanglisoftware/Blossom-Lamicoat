  ...
  const initTabulator = () => {
    ...
    tabulator.current = new Tabulator(tableRef.current, {
      ...
      columns: [
        ...
        { title: "GSM/GLM", field: "gsmMasterName", hozAlign: "center",  headerHozAlign: "center",minWidth: 200 },
            ...
            const actions = [
              {
                ...
                onClick: () => {
                  setEditingQualityId(rowData.id);
                  setEditQualityModal(true);
                },
              },
              {
                label: "Delete",
...
const handleFilterChange = (value: string) => {
  ...
  const handleDeleteFproduct = async () => {
    if (!deleteQualityId) return;

    try {
      await axios.delete(`${BASE_URL}/api/quality/${deleteQualityId}`, {
        headers: { Authorization: token ? `Bearer ${token}` : "" },
      });
  ...
  return (
    <>
      <div className="flex items-center justify-between mt-8 mb-4">
        ...
        <Button variant="primary" onClick={() => setAddQualityModal(true)}>
          Add Product
        </Button>
      </div>
      ...
      <AddGoods
        open={addQualityModal}
        onClose={() => setAddQualityModal(false)}
        onSuccess={refreshTable}
      />

      <EditGoods
        open={editQualityModal}
       QualityId={editingQualityId}
        onClose={() => setEditQualityModal(false)}
        onSuccess={refreshTable}
      />
      <Dialog
